...
trait EncryptVerifyCodeTrait
{
    ...
    protected function encrypt(AuthenticateToken $token): string
    {
        return $this->hash256(strtr('{ha1}:{nonce}:{cnonce}:{qop}:{message}:{ha2}', [
            ...
            '{cnonce}' => $token->cnonce(),
            '{qop}' => $token->qop,
            '{message}' => $this->getVerifyRequestContent(),
            '{ha2}' => $this->hash2()
        ]));
    }
    ...
    public function getRequestContent(): string
    {
        return str_replace(
            ["\n", '<?xml version="1.0"?>'],
            '',
            $this->toXml(array_merge(
                $this->request->validate()->toArray(), [
                'ShopNO' => $this->companyId,
                'KeyNum' => $this->key,
            ]), $this->request->getXmlHeader())
        );
    }
     ...
     * @return string
     */
    public function getVerifyRequestContent(): string
    {
        return str_replace(' ', '', $this->getRequestContent());
    }
